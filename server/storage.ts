...
export class DatabaseStorage implements IStorage {
	...
	// Receipt methods implimentation
	...
	//Fetch all Receipts
	async getReceipts(): Promise<(Receipts & { assignedUser?: Pick<User, 'id' | 'username'> })[]> {
		try {
			return await executeWithRetry(async () => {
				const items = await db
					.select({
					...
					.from(receipt)
					.leftJoin(users, eq(receipt.user_id, users.id))
			...
			throw new DatabaseError(`Failed to get receipt: ${error instanceof Error ? error.message : String(error)}`);
		}
	}
	...
	async createReceipt(
		...
		return await executeWithRetry(async () => {
			return await db.transaction(async (tx) => {
				...
				await tx.insert(receipt_items).values(receiptItems);
