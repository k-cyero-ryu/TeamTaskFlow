...
import { Plus, Edit, Trash2, ArrowLeft, Phone, Mail, MessageCircle, Building, User, FileText, Upload } from 'lucide-react';
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <p className="text-sm text-gray-600">{service.contractFile?.split('/').pop() || 'No file'}</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            />
            {uploadedFilePath && (
              <p className="text-sm text-green-600">✓ New file uploaded: {uploadedFilePath.split('/').pop()}</p>
            )}
          </div>
...
export default function ClientDetails({ params }: { params: { id: string } }) {
  ...
  const { data: clientServices = [], isLoading: servicesLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiRequest('GET', `/api/client-services/client/${clientId}`);
  ...
  const removeMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/client-services', clientId] });
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
              <span className="font-medium">Start Date:</span> {format(new Date(client.startDate), 'MMM d, yyyy')}
            </div>
            {contactInfo?.email && (
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm text-gray-600">
                        ...
                        {item.client_services.notes && (
                          <div className="col-span-2">
                            ...
                            <p className="text-gray-600 mt-1">{item.client_services.notes}</p>
                          </div>
                        )}
                      </div>
                    </div>
